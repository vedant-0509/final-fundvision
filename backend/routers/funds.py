...
async def list_funds(
    ...
    # Sorting Logic (Fixed for MySQL Compatibility)
    sort_col = getattr(Fund, sort_by, Fund.cagr_5y)
    if sort_order == "desc":
        # REMOVED .nulls_last() because MySQL syntax doesn't support it
...
async def refresh_nav(
    ...
    for fund in funds:
        if fund.scheme_code in nav_map:
            ...
            fund.nav = nav_val
            try:
                # Assuming AMFI format DD-MMM-YYYY or ISO
                fund.nav_date = date.fromisoformat(nav_date_str)
            except Exception:
                pass
            updated += 1
