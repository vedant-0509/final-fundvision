        ...
        {tab === 'news' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-gray-900">Market News Sentiment Analysis</h2>
            ...
            {newsLoading ? (
              <div className="bg-white rounded-xl border border-gray-200 p-12 text-center shadow-sm">
                ...
                <p className="text-gray-500 text-sm">Analysing market sentiment…</p>
              </div>
            ) : news ? (
              <>
                {/* Overall sentiment */}
                <div className={`rounded-xl border p-5 mb-5 shadow-sm ${SENTIMENT_BG[news.overall.label]}`}>
                  <div className="flex items-center gap-3">
                    <Newspaper className={`w-6 h-6 ${SENTIMENT_COLOR[news.overall.label]}`} />
                    <div>
                      <p className="font-semibold text-gray-900">Overall Market Sentiment: <span className={`capitalize ${SENTIMENT_COLOR[news.overall.label]}`}>{news.overall.label}</span></p>
                      <p className="text-sm text-gray-500">Composite score: {(news.overall.score * 100).toFixed(0)}/100 (based on {news.sentiments.length} headlines)</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {news.sentiments.map((s, i) => (
                    <div key={i} className={`bg-white rounded-xl border p-4 shadow-sm ${SENTIMENT_BG[s.sentiment]}`}>
                      <div className="flex items-start gap-3">
                        <div className={`w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0 ${SENTIMENT_DOT[s.sentiment]}`} />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 mb-1">{s.headline}</p>
                          <p className="text-xs text-gray-500 mb-2">{s.impact}</p>
                          <div className="flex items-center gap-3">
                            <span className={`text-xs font-semibold capitalize ${SENTIMENT_COLOR[s.sentiment]}`}>{s.sentiment}</span>
                            <div className="flex items-center gap-1">
                              <div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                <div className={`h-full rounded-full ${s.score > 0 ? 'bg-emerald-500' : 'bg-red-500'}`}
                                  style={{ width: `${Math.abs(s.score) * 100}%` }} />
                              </div>
                              <span className="text-xs text-gray-400">{s.score > 0 ? '+' : ''}{s.score.toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
